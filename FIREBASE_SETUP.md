...
| **App closes right after login** (Android only) | The Capacitor push plugin's `register()` ran without a `FirebaseApp`. Its first statement is `FirebaseMessaging.getInstance()` on the main thread, which throws and kills the process — so it looks like a crash, not an error. Cause: `android/app/google-services.json` is missing, so `android/app/build.gradle` (lines 72–79) never applies the `com.google.gms.google-services` plugin. This build now **detects that at build time** and skips native push instead of crashing (see `vite.config.ts` → `__ANDROID_FIREBASE_CONFIGURED__` → `isNativePushUsable()`), and the **Android Firebase config** row in Push Diagnostics goes red. To re-enable push, add `google-services.json` at `android/app/` and rebuild. |
